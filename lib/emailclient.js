...
function listen(imapConnection, newMessageCallback) {
    ...
    /// </summary>
    /// <param name="imapConnection">The configuration for the imap server connection.</param>
    ...
    imapConnection.on('error', onError);
    ...
    function onConnect(error) {
        ...
        if (error) {
            log.warn("An error occured connecting.");
        ...
        try {
            ...
            log.error("Error opening mailbox: " + e);
        }
    }
    ...
    function onBoxOpen(error, mailbox) {
        ...
        if (error) {
            log.error("An error occured opening mailbox: " + error);
    ...
    function onMailChecked(error, results) {
        ...
        if (error) {
            log.error("An error occured searching the mailbox for new mail.");
        ...
        log.verbose(itemsFound.toString() + " items are available.");
    ...
    function fetchingMessage(message) {
        ...
        log.verbose("Message #" + message.seqno + " is being fetched.");
    ...
    function onAllMessagesFetched(error) {
        ...
        // If there was an error.
        if (error) {
            log.error("An error occured retreiving messages.");
    ...
    function reconnect(hadError) {
        ...
        if (!busyConnecting && !reconnecting) {
            ...
            log.warn("Connection lost. Attempting reconnect in 5 seconds.");
    ...
    function onLogout(error) {
        ...
        log.info("Logged out. Reason: " + reason);
    }

    function onError(error) {
        if (error.code === "ECONNRESET")
            log.warn("Connection to server reset.")
        else
            log.warn("Connection error. " + error.stack);
    }
}
