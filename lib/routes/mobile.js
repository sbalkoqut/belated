...
function create(meetingStore)
{
    function getMeetings(request, response) {
        ...
        if (!utils.isString(email) || !email.match(validEmail)) {
            log.warn("Error getting meetings: email invalid.");
            response.send(400);
            return;
        }
        ...
        meetingStore.findMeetingsFor(email, function (error, items) {
            if (error) {
                log.error("Unable to query for meetings with particular attendee. " + error);
                response.send(500);
                return;
            }
    ...
    function setTravelPlan(request, response)
    {
        ...
        if (!validate.email(email)) {
            log.warn("Error updating travel plan: User email is invalid.");
            response.send(400);
            return;
        }

        if (!validate.travel(mode, eta)) {
            log.warn("Error updating travel plan: Travel mode and/or eta is invalid.");
            response.send(400);
            return;
        }
        ...
        meetingStore.get(meetingId, function (error, meeting) {
            if (error) {
                log.error("Unable to query for meeting needed to update travel plan. " + error);
                response.send(500);
                return;
            }
            if (meeting === null) {
            ...
            meetingStore.updateTravelPlan(meeting, travelPlan, function (error) {
                if (error) {
                    log.error("Unable to update meeting with new travel plan.");
                    response.send(500);
                    return;
                }
                response.send(200);
