...
function create(mongodb, callback) {
    ...
    //    attendees: [ { name: [String], email: [String], notifiedLate: [Boolean], track: [Boolean], deleted: [Boolean] }, ... ],
    //    subject: [String],
    //    description: [String],
    //    emailId: [String],
    //    calUId: [String],
    ...
    function sanitise(meeting, options)
    {
        ...
        for (var i = 0; i < meeting.attendees.length; i++) {
            ...
            if (attendee.deleted !== undefined && !utils.isBoolean(attendee.deleted))
                throw new TypeError("Meeting attendee has an invalid deleted flag value.");
        ...
        if (meeting.organiser.notifiedLate === true && options.ignoreNotifiedLate !== true)
            record.organiser.notifiedLate = true;


        record.organiser = replaceObjectContent(meeting.organiser, record.organiser);

        for (var i = 0; i < meeting.attendees.length; i++) {
            var attendee = { name: meeting.attendees[i].name, email: meeting.attendees[i].email, notifiedLate: false, track: meeting.attendees[i].track, deleted: false };
            if (meeting.attendees[i].notifiedLate === true && options.ignoreNotifiedLate !== true)
                attendee.notifiedLate = true;
            if (meeting.attendees[i].deleted === true && options.ignoreDeleted !== true)
                attendee.deleted = true;

            attendee = replaceObjectContent(meeting.attendees[i], attendee);
    ...
    function add(meeting, callback) {
        ...
        var record = sanitise(meeting, {
            ...
            ignoreDeleted: true,
            ignoreGeocodingRequired: true,
            ignoreConfirmationRequired: true
        });
    ...
    function updateDetail(meeting, detail, callback)
    {
        function delta(existing, updated)
        {
            ...
            {
                function addAttendeeDelta(update, i, existingAttendee, updatedAttendee, startTimeChanged)
                {
                    var attendeePrefix;
                    var included = true;
                    if (i !== null) {
                        attendeePrefix = "attendees." + i.toString();
                        included = !(existingAttendee.deleted === true);
                        var include = (updatedAttendee !== null);

                        if (included !== include) {
                            update.$set[attendeePrefix + ".deleted"] = !include;
                        ...
                        attendeePrefix = "organiser";

                    if (existingAttendee.name !== updatedAttendee.name)
                    {
                        update.$set[attendeePrefix + ".name"] = updatedAttendee.name;
                    ...
                    if ((startTimeChanged || !included) && existingAttendee.notifiedLate) {
                        update.$set[attendeePrefix + ".notifiedLate"] = false;
                        existingAttendee.notifiedLate = false;
                    }
                    return update;
                }

                update = addAttendeeDelta(update, null, existing.organiser, updated.organiser, startTimeChanged);
                ...
                existing.attendees.forEach(function (existingAttendee, i) {
                    ...
                    update = addAttendeeDelta(update, i, existingAttendee, updatedAttendee, startTimeChanged);
                });
    ...
    function findMeetingsWithin(earliestStart, latestStart, callback) {
        ...
        {
            callback(error);
            log("Error finding meetings in time range. " + error)
        }
    }
    ...
    function findMeetingsFor(email, callback)
    {
        ...
        {
            callback(error);
            log("Error finding meetings for email. " + error)
        }
    }
    ...
    function findMeetingBy(organiserEmail, calUId, callback)
    {
        function deleteExcess(meetings) {
            try {
                ...
                log("Error cleaning up excess meetings." + e);
            }
        }
        ...
        try
        {
            collection.find({
                ...
                if (success && items.length > 1) {
                    log("Excess meetings found for given organiser email & calender UID, cleaning up.");
                    deleteExcess(items);
                }
                ...
                try
                {
                    ...
                    log("Error in callback after #findMeetingBy. " + e);
                }
            });
        }
        catch (error)
        {
            callback(error);
            log("Error finding meeting for organiser email and calendar UID. " + error)
        }
    }
