...
function start(configuration) {
    ...
    persistancy.connect(function (error, dataStores) {
        if (error) {
            log.error("Failed to connect to persistency. " + error);
            process.exit(1);
        }
        log.info("Connected to persistency.");
    ...
    function setupMeetingReceiver(notificationManager)
    {
        emailClient(
            ...
            emailHandler(function (error, method, meeting) {
                if (error)
                    log.warn("A meeting wasn't handled. " + error.message);
                else {
                    log.info("Meeting received: " + inspect(meeting));
                    if (method === "request")
                        notificationManager.handleMeetingRequest(meeting);
                    else if (method === "cancel")
                        notificationManager.handleMeetingCancellation(meeting);
                    else {
                        log.warn("Meeting had an invalid method: " + method);
                    }
                }}),
            true);
    }
    ...
    function setupLocationReceiver(notificationManager)
    {
        app.all("/REST/location", locationHandler(function (error, location) {
            if (error)
                log.warn("A location update failed. " + error);
            else {
                log.info("Location received: " + inspect(location));
    ...
    function startWeb()
    {
        ...
        httpServer.on("error", function (error) {
            ...
            if (error.code == "EADDRINUSE") {
                log.error("Couldn't start HTTP server: The port in use.");
            }
            else if (error.code == "EACCES") {
                log.error("Couldn't start HTTP server: Root privileges required to bind port. Try running with 'sudo'.")
            }
            else {
                log.error("Problem with HTTP server:");
                log.error(error);
            }
            process.exit(1);
        });
