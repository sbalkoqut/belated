...
function geocode(location, callback) {
    geocodeBing(location, callback, 0);
}
...
function geocodeBing(location, callback, attemptNumber)
{
    ...
    var request = http.get(url, function (response) {
        ...
        response.on("end", function () {
            var result;
            try {
                var rateLimited = response.headers['x-ms-bm-ws-info'];
                if (rateLimited === "1") {
                    if (attemptNumber >= 6) {
                        ...
                        log("Geocoding is rate limited, trying again in 10 seconds (attempt #" + attemptNumber + ")/")
                        setTimeout(function () {
                            geocodeBing(location, callback, attemptNumber);
                        }, 10000);
                    }
                }
                else {
