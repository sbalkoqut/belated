...
function start(mStore, notificationLogic) {
    ...
    function scheduleNotifications(meeting) {
        ...
        if (!scheduled) {
            log.verbose("Didn't schedule meeting organised by " + meeting.organiser.name + ", already occurred.");
            mStore.remove(meeting, function (error) {
                if (error) {
                    log.error("Failed to remove meeting " + meeting._id + ": " + error);
                }
            });
            return false;
        }
        else {
            log.verbose("Scheduled for meeting organised by " + meeting.organiser.name + ".");
            return true;
        }
    }
    ...
    function checkMeeting(meeting, minutesBeforeStart) {
        mStore.get(meeting._id, function (error, currentMeeting) {
            if (error)
            {
                log.error("Unable to get retreive meeting for notifications." + error);
                return;
            }
            if (!currentMeeting || (meeting.calSequence !== currentMeeting.calSequence))
            {
                log.verbose("Meeting updated since meeting check scheduled. Skipping.");
                return;
            }
            notificationLogic.statusUpdate(currentMeeting, minutesBeforeStart, function (newPersonsLate) {

                if (minutesBeforeStart === 0) {
                    log.verbose("Removing a meeting that just started.");
                    mStore.remove(currentMeeting, function (error) {
                        if (error) {
                            log.error("Failed to remove meeting " + currentMeeting._id + ": " + error);
                        }
                    });
                }
                else if (newPersonsLate.length > 0) {
                    log.verbose("Persisting late attendees for meeting organised by " + currentMeeting.organiser.name + ".");
                    mStore.updateNotifiedLatePersons(currentMeeting, newPersonsLate, function (error) {
                        if (error) {
                            log.error("Failed to persist persons notified late. " + error);
                        }
                    });
                }
            });
